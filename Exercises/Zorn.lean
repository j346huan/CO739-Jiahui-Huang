...
theorem IsChain.le_or_lt_of_mem_of_mem (hC : IsChain C) (hxC : x ∈ C) (hyC : y ∈ C) :
    x ≤ y ∨ y < x := by
  have h0:= IsChain.le_or_le_of_mem_of_mem hC hxC hyC
  obtain (h1|h2) := h0
  left
  exact h1
...
theorem IsChain.subset_isChain {C S : Set α} (hC : IsChain C) (hSC : S ⊆ C) : IsChain S :=
  fun (x y:α ) (hx: (x∈ S)) (hy: (y ∈  S))=>hC x y (mem_of_mem_of_subset hx hSC) (mem_of_mem_of_subset hy hSC)
...
theorem IsSegmentOf.chain_left (h : IsSegmentOf S C) : IsChain S :=
  IsChain.subset_isChain (IsSegmentOf.chain_right h) (IsSegmentOf.subset h)
...
theorem IsSegmentOf.subset_right (h : IsSegmentOf S C) (hD : IsChain D) (hSD : S ⊆ D)
  ...
  ⟨hSD, hD, fun (x y:α) (hxD: (x∈ D)) (hyS: (y ∈  S)) (hxy:x≤y) => IsSegmentOf.mem_of_mem_of_le h (mem_of_mem_of_subset hxD hDC) hyS hxy⟩
...
theorem IsSegmentOf.trans (h : IsSegmentOf S C) (h' : IsSegmentOf C D) : IsSegmentOf S D :=
  ⟨subset_trans  h.subset h'.subset ,h'.chain_right, fun (x y:α) (hxD: (x∈ D)) (hyS: (y ∈  S)) (hxy:x≤y) => h.mem_of_mem_of_le (h'.mem_of_mem_of_le hxD (mem_of_mem_of_subset hyS (h.subset)) hxy) hyS hxy ⟩
...
theorem IsSegmentOf.exists_strictUB_mem_of_ne (h : IsSegmentOf S U) (hne : S ≠ U) :
  ...
  have h_ssubset : S ⊂ U := by
    apply subset at h
    exact ssubset_of_ne_of_subset hne h
  ...
  have le_or_lt : d ≤ c ∨ c < d := by
    have hU: IsChain U := h.chain_right
    exact hU.le_or_lt_of_mem_of_mem hdU (mem_of_mem_of_subset hcS (subset h))

  obtain (hdc | hcd) := le_or_lt
  · -- Use the fact that `S` is a segment of `U` to get a contradiction in this case.
    have hdS':=h.mem_of_mem_of_le hdU hcS hdc
    contradiction
  exact hcd
...
theorem Good.chain_right (h : GoodWRT b C) : IsChain C :=
  h.1
  -- This proof should be five characters long.
...
theorem goodWRT_comparability {C C' : Set α} {b : Set α → α} (hb : ∀ C, IsChain C → b C ∉ C)
  ...
  have hmutSC : ∀ S ∈ mutualSegs, IsSegmentOf S C := by
    intro S hS
    exact hS.1
  have hmutSC' : ∀ S ∈ mutualSegs, IsSegmentOf S C' := by
    intro S hS
  ...
  have hUC : IsSegmentOf U C := IsChain.sUnion_segmentOf (Good.chain_right hC) mutualSegs hmutSC
  have hUC' : IsSegmentOf U C' := IsChain.sUnion_segmentOf (Good.chain_right hC') mutualSegs hmutSC'
  ...
  have hCseg : IsSegmentOf (insert (b U) U) C := hC.2 U hUC hUC_eq
  have hC'seg : IsSegmentOf (insert (b U) U) C' := hC'.2 U hUC' hUC'_eq

  apply hb _ hUC.chain_left
  have hbU : insert (b U) U ⊆ U :=
    -- use the definition of `U` and `subset_sUnion_of_mem` to prove this
    subset_sUnion_of_mem ⟨hCseg,hC'seg⟩
...
theorem GoodWRT_sUnion_chain (b : Set α → α) (hb : ∀ C, IsChain C → b C ∉ C) :
  ...
  · -- use the fact that `Sx` is a chain.
    apply Good.chain_right at hSx
    apply IsSegmentOf.subset at h
    apply h at hySy
    exact IsChain.le_or_le_of_mem_of_mem hSx hxSx hySy
  -- use the fact that `Sy` is a chain.
  apply Good.chain_right at hSy
  apply IsSegmentOf.subset at h
  apply h at hxSx
  exact IsChain.le_or_le_of_mem_of_mem hSy hxSx hySy
...
theorem GoodWRT.insert_ub (b : Set α → α) (hb : ∀ C, IsChain C → IsStrictUB C (b C))
  ...
  have hbCS : b C ∉ S := by
    ...
    have hcbC : c≤ b C := by
      apply hbCUB at hc
      exact hc
    apply mem_insert_of_mem (b C) at hc
    exact IsSegmentOf.mem_of_mem_of_le hSeg hc hbCS hcbC
  ...
  have hSC : IsSegmentOf S C := by
    refine ⟨?_, ?_, ?_⟩
    exact hS
    exact Good.chain_right h
    intro x y hx hy hxy
    apply mem_insert_of_mem (b C) at hx
    exact hSeg.2.2 x y hx hy hxy
  ...
  · -- Since `b C ≤ y ∈ S ∪ {b s} ⊆ C`, this contradicts `b C` being a strict UB for `C`.
    have hy':=  (mem_of_subset_of_mem hSeg'.subset hy)
    apply (hb C h.1) at hy'
    apply not_lt_of_le at hxy
    contradiction
...
theorem zorn (h : ∀ C, IsChain C → ∃ (b : α), IsUB C b) : ∃ (m :α ), IsMaximal m := by
  ...
  have h_strictUB : ∀ (C : Set α), ∃ (b : α), IsChain C → IsStrictUB C b := by
    ...
    · -- use `h` and `h_con` to find a strict upper bound.
      let ⟨b0, hb0⟩:=h C hC
      let ⟨b1, hb1⟩:=h_con b0
      have hbs := lt_of_le_of_ne hb1.1 hb1.2
      use b1
      intro ?_ c hc
      apply hb0 at hc
      apply lt_of_le_of_lt hc hbs
  ...
  have forall_good_segment : ∀ D, GoodWRT b D → IsSegmentOf D U := by
    ...
    intro D hD
    refine ⟨subset_sUnion_of_mem hD,hU_chain,?_⟩
    intro x y hx hy hxy
    rw[mem_sUnion]  at hx
    let ⟨T, hT, hxT⟩ := hx
    have hDT := goodWRT_comparability hb_not_mem hD hT
    obtain (h|h) := hDT
    apply IsSegmentOf.subset at h
    exact h hxT
    exact IsSegmentOf.mem_of_mem_of_le h hxT hy hxy
  ...
  have hU_good : GoodWRT b U := by
    ...
    have hSD : S ⊆ D := by
      intro s hs
      have hs1:= hSd s hs
      apply le_of_lt at hs1
      apply hDU.2.2 at hs1
      exact hs1
      apply (IsSegmentOf.subset hS) at hs
      exact hs
      exact hdD

    have hSD_seg : IsSegmentOf S D :=
      IsSegmentOf.subset_right hS (Good.chain_right hD) hSD (IsSegmentOf.subset hDU)
    ...
    have hSneD : S ≠ D := by
      ...
      apply IsStrictUB.not_mem at hSd
      tauto
  ...
  have hU_ins_good := hU_good.insert_ub b hb

  -- But this means that `U ∪ {b U}` is a subset of `U`, ...
  have h_ins : insert (b U) U ⊆ U := by
    exact subset_sUnion_of_mem hU_ins_good

  -- ... which contradicts `b U ∉ U`.
  apply insert_subset_iff.1 at h_ins
  have hbU:= hb_not_mem U hU_chain
  tauto
