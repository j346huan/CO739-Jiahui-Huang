...
theorem WOSet.le_or_lt (hx : x ∈ W.S) (hy : y ∈ W.S) : W.le x y ∨ W.lt y x := by
  ...
  have hA: A.Nonempty:= by
    use x
    rw[hA_def, mem_insert_iff]
    tauto

  have hxy:= WOSet.exists_min W A hxyS hA
  obtain ⟨b,hb,ht⟩ := hxy
  obtain (hx|hby) :=hb
  left
  rw[hx] at ht
  have hy: y∈ A:= by
    ...
    tauto
  exact ht y hy
  rw[ mem_singleton_iff] at hby
  obtain (heq|hne):= eq_or_ne x y
  left
  rw[heq]
  apply W.refl
  exact hy
...
theorem WOSet.eq_iff : W = W' ↔ W.S ⊆ W'.S ∧ ∀ x y, W'.le x y → W.le x y := by
  ...
  intro hxy
  by_contra hxy'
  rw[le_iff_not_lt,not_not] at hxy'
  obtain ⟨ h1,h2⟩ :=hxy'
  have hyx: W.lt y x:= ⟨ h y x h1,h2⟩
  rw[lt_iff_not_le] at hyx
  tauto
  exact hxy.mem_right
  exact hxy.mem_left
  exact mem_of_subset_of_mem hss hxy.mem_left
  exact mem_of_subset_of_mem hss hxy.mem_right
  exact h x y
...
theorem IsInitialSegment.le_iff_le (h : IsInitialSegment W W') (hy : y ∈ W.S) :
  ...
  constructor
  exact h.le_of_le
  have hx:= h.eq_setOf_lt
  intro hxy
  obtain ⟨a,_, ha⟩ :=hx
  have hyW: y∈ {y|W'.lt y a}:=by
    rw[ha.symm]
    exact hy
  have hya: W'.lt y a :=hyW
  have hxa: W'.lt x a:= hxy.trans_lt hya
  have hx: x∈ W.S:= by
    rw[ha]
    apply hxa
  obtain (h1|h2):= W.le_or_lt hx hy
  exact h1
  obtain ⟨h1,h2⟩ :=h2
  apply h.le_of_le at h1
  apply hxy.antisymm at h1
  rw[h1] at h2
  contradiction
...
theorem IsInitialSegment.lt_iff_lt (h : IsInitialSegment W W') (hy : y ∈ W.S) :
  ...
  constructor
  intro hxy
  have hle: W'.le x y:= ((h.le_iff_le hy).1) hxy.le
  exact ⟨hle, hxy.ne⟩
  intro hxy
  have hle: W.le x y:= ((h.le_iff_le hy).2) hxy.le
  exact ⟨hle, hxy.ne⟩
...
theorem IsInitialSegment.trans {W₁ W₂ W₃ : WOSet α} (h : IsInitialSegment W₁ W₂)
  ...
  constructor
  · intro x y hxy
    apply h'.le_of_le (h.le_of_le hxy)
  have h23: W₂.S⊆ W₃.S:= h'.subset
  have h3:= h23 hx₂
  use x₂
  constructor
  exact h3
  rw[hW₁]
  simp_rw [h'.lt_iff_lt hx₂]
...
theorem WOSet.le_alt (h : ∀ {x y}, W.le x y → W'.le x y)
  ...
  · left
    ...
    exact hss
    intro x y hxy
    have hyW':= hxy.mem_right
    have h1:=h_seg x y hxy
    have hyW:= hss' hyW'
    have hxW:=h1 hyW
    have hlet:=WOSet.le_or_lt hxW hyW
    obtain (hle|hlt):= hlet
    exact hle
    have hle:=hlt.le
    apply h at hle
    apply lt.ne at hlt
    have hne:=le.antisymm hle hxy
    tauto
  ...
  rw[mem_setOf]
  intro hyx
  have hyW':= hyx.le.mem_left
  by_contra h'
  apply hx at h'
  rw[W'.lt_iff_not_le] at hyx
  tauto
  exact hyW'
  exact hxW'
  exact hyW'
...
def WOSet.insertAbove (W : WOSet α) (a : α) (ha : a ∉ W.S) : WOSet α where
  ...
  antisymm := by
    ...
    rintro x y (hxy | ⟨(rfl | _?), rfl⟩) (hyx | ⟨(hy | _?), hxa⟩)
    · exact W.antisymm _ _ hxy hyx
    · simp[hxa,hy]
    apply le.mem_left at hxy
    rw[hxa] at hxy
    tauto
    rfl
    rfl
    rfl
    apply le.mem_left at hyx
    tauto
    exact hxa
    exact hxa
  ...
  trans := by
    ...
    exact hyz.right
    right
    apply le.mem_left at hyz
    rw[hxy.right] at hyz
    tauto
    right
    exact ⟨hxy.left,hyz.right⟩
  ...
  -- (So we need that it is a nonempty subset of `W.S`)
    ...
    have hne : (T \ {a}).Nonempty := by
      ...
      by_contra hne
      rw[diff_eq_empty] at hne
      rw[hTne.subset_singleton_iff] at hne
      tauto
...
def unionChain (Ws : Set (WOSet α)) (hWs : IsChain Ws) : WOSet α where
  ...
  supportedOn := by
    ...
    constructor
    use W
    simp [hW,hxy.mem_left,hxy.mem_right]
    use W
    simp [hW,hxy.mem_left,hxy.mem_right]
  ...
  trans := by
    ...
    have h_cases := hWs.eq_or_segment_or_segment hW hW'
    obtain (h|h|h):=h_cases
    rw[← h] at hyz hW'
    use W
    exact⟨hW, hxy.trans hyz⟩
    use W'
    rw[h.le_iff_le] at hxy
    exact ⟨ hW', hxy.trans hyz⟩
    exact hxy.mem_right
    use W
    rw[h.le_iff_le] at hyz
    exact ⟨ hW, hxy.trans hyz⟩
    exact hyz.mem_right
  ...
  exists_min := by
    ...
    have hW : ∃ W ∈ Ws, (W.S ∩ T).Nonempty := by
      ...
      obtain ⟨W,hW,hxW⟩ :=h
      apply hcon at hW
      have hxi:= (mem_inter_iff x W.S T).2 ⟨hxW,hxT⟩
      rw[hW] at hxi
      tauto
    ...
    have hseg : IsInitialSegment W W' := by
      ...
      tauto
      tauto
      apply IsInitialSegment.subset at h
      apply h at htW'
      tauto
    use W', hW'
    apply IsInitialSegment.eq_setOf_lt at hseg
    obtain ⟨x,hx,hxW ⟩:=hseg
    rw[hxW] at hbW  htW
    simp only [mem_setOf_eq] at htW hbW
    rw[← W'.le_iff_not_lt] at htW
    exact W'.trans b x t  hbW.le htW
    trivial
    trivial
...
theorem unionChain_isUB (Ws : Set (WOSet α)) (hWs : IsChain Ws) :
  ...
  have h : ∃ W' ∈ Ws, IsInitialSegment W W' := by
    ...
    have hS : W.S = (unionChain Ws hWs).S := by
      ...
      intro W' hW'
      have h':= h' W' hW'
      have hWW':=hWs.eq_or_segment_or_segment hW hW'
      obtain(h|h|h):=hWW'
      rw[h]
      tauto
      exact h.subset
    ...
    intro hW'
    have hy: y∈ W.S:= by
      apply WOSet.le.mem_right at hW'
      rwa[← hS] at hW'
    simp at hW'
    obtain ⟨ W',hW', hxy⟩ :=hW'
    exact hWs.le_of_le hW' hW hxy hy
  ...
  intro h
  obtain ⟨W'',hW'',hyx ⟩:=h
  exact ⟨hWs.le_of_le hW'' hW' hyx.le hx, hyx.ne⟩
...
theorem exists_WOSet_on_univ (α : Type*) : ∃ (wo : WOSet α), wo.S = univ := by
  ...
  suffices hmax : ∃ (W : WOSet α), IsMaximal W by
    ...
    obtain ⟨wo, hmax⟩:=hmax
    apply eq_univ_of_maximal at hmax
    use wo

  -- I know how to prove there is a maximal set!
  apply zorn
  intro Ws hWs
  use (unionChain Ws hWs)
  simp[unionChain_isUB]
...
noncomputable def WOSet.toWellOrder (W : WOSet α) (hW : W.S = univ) :
  ...
  exists_min := by
    intro S
    have hS:= subset_univ S
    rw[← hW] at hS
    exact W.exists_min  S hS
